...
class TestQJLProjection:
    ...
    def test_pack_reduces_memory(self, qjl):
        ...
        packed = qjl.pack_signs(signs)
        # Each packed byte holds 8 signs
    ...
    def test_decode_correction_shape(self, qjl):
        ...
        x = torch.randn(10, 128)
        signs = qjl.encode(x)
        correction = qjl.decode_correction(signs)
    ...
    def test_inner_product_estimation(self, qjl):
        ...
        signs_a = qjl.encode(a.unsqueeze(0))
        signs_b = qjl.encode(b.unsqueeze(0))
        estimated_ip = qjl.compute_inner_product_estimate(signs_a, signs_b)

        # The estimate should have the same sign as the true IP
        # (with high probability for 256 projections)
        # We just check it's not wildly off
        assert abs(estimated_ip.item()) < 2.0  # Bounded
    ...
    def test_correction_reduces_error(self):
        ...
        pq = PolarQuant(dim=128, bits=2)  # Low bits = more residual
        qjl = QJLProjection(input_dim=128, projection_dim=512)

        torch.manual_seed(42)
        x = torch.randn(50, 128)

        # Encode with PolarQuant
        encoded = pq.encode(x)
        reconstructed = pq.decode(encoded)

        # Compute residual and QJL correction
        ...
        # QJL-corrected should have lower error for most vectors
        error_before = ((x - reconstructed) ** 2).sum(dim=-1)
        error_after = ((x - corrected) ** 2).sum(dim=-1)

        # At least some vectors should improve (QJL is noisy)
